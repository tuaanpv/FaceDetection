...
int main(int argc, char** argv) try
{
    if (argc != 3)
    {
        ...
        cout << "   ./dnn_face_recognition_ex 0.5 faces/bald_guys.jpg" << endl;
        cout << endl;
    ...
    cout << "number of faces found in the image: "<< faces.size() << endl;
    ...
    for (size_t i = 0; i < face_descriptors.size(); ++i)
    {
        for (size_t j = i+1; j < face_descriptors.size(); ++j)
        {
            ...
            if (length(face_descriptors[i]-face_descriptors[j]) < face_Threshold)//0.6)
            {
    ...
    cout << "number of edges.size() before chinese_whispers: "<< edges.size() << endl;

    std::vector<unsigned long> labels;
    const auto num_clusters = chinese_whispers(edges, labels);
    // This will correctly indicate that there are 4 people in the image.
    cout << "number of people found in the image after chinese_whispers: "<< num_clusters << endl;
